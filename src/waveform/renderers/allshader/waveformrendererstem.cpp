...
//     const int reserved = numVerticesPerLine *
//             (mixxx::audio::ChannelCount::stem() * 2 * stripLength + 1);
...
//                         // // 1. Skip proxy lookup entirely for the premix layer (stemIdx == 0)
//                         // // 2. Ensure we stay within array bounds 
//                         // // 3. Use the real Mixxx API call '.valid()' to ensure the control is initialized
//                         // if (stemIdx > 0 && 
//                         //     !m_pStemGain.empty() && 
//                         //     stemIdx < static_cast<int>(m_pStemGain.size()) && 
//                         //     m_pStemGain[stemIdx] &&
//                         //     m_pStemGain[stemIdx]->valid()) { // <--- The real Mixxx method!

//                         // 1. Completely bypass volume proxy evaluation for the premix layer (stemIdx == 0)
//                         // 2. Bound-check the vector size safely
//                         // 3. Dereference the pointer (*m_pStemGain[stemIdx]) or use -> to invoke .valid() 
//                         //    on the actual ControlProxy object.
//                         if (stemIdx > 0 && 
//                             stemIdx < static_cast<int>(m_pStemGain.size()) && 
//                             m_pStemGain[stemIdx]->valid()) { // <--- Dereferences the proxy to call valid()
