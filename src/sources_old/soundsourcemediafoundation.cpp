...
SoundSource::OpenResult SoundSourceMediaFoundation::tryOpen(
    ...
    static_assert(sizeof(wchar_t) == sizeof(ushort),
            "QString::utf16(): wchar_t and ushort have different sizes");
...
void SoundSourceMediaFoundation::seekSampleFrame(SINT frameIndex) {
    ...
    if (isValidFrameIndex(m_currentFrameIndex) && (m_currentFrameIndex < frameIndex)) {
        ...
        if (skipFrames.length() <= skipFramesCountMax) {
            if (skipFrames !=
                    readSampleFramesClamped(WritableSampleFrames(skipFrames))
                            .frameIndexRange()) {
                kLogger.warning()
    ...
    if (m_currentFrameIndex != frameIndex) {
        ...
        if (SUCCEEDED(hrSetCurrentPosition)) {
            ...
            // See also:
            // https://msdn.microsoft.com/en-us/library/windows/desktop/dd374668(v=vs.85).aspx
            //   "The SetCurrentPosition method does not guarantee exact
            //   seeking." ... "After seeking, the application should call
            ...
            if (skipFrames.empty()) {
                ...
                if (skipFrames !=
                        readSampleFramesClamped(
                                WritableSampleFrames(skipFrames))
                                .frameIndexRange()) {
                    kLogger.warning()
...
ReadableSampleFrames SoundSourceMediaFoundation::readSampleFramesClamped(
    ...
    while (numberOfFramesRemaining > 0) {
        ...
        while (dwSampleBufferIndex < dwSampleBufferCount) {
            ...
            HRESULT hrGetBufferByIndex = pSample->GetBufferByIndex(
                    dwSampleBufferIndex, &pMediaBuffer);
