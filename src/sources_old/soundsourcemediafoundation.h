  ...
  public:
    ...
    explicit StreamUnitConverter(const AudioSource* pAudioSource)
            : m_pAudioSource(pAudioSource),
              m_fromSampleFramesToStreamUnits(double(kStreamUnitsPerSecond) /
                      double(pAudioSource->getSignalInfo().getSampleRate())),
              m_fromStreamUnitsToSampleFrames(
                      double(pAudioSource->getSignalInfo().getSampleRate()) /
                      double(kStreamUnitsPerSecond)) {
    ...
    SINT toFrameIndex(LONGLONG streamPos) const {
        ...
        // sample frames.
        const SINT frameIndexOffset = static_cast<SINT>(
