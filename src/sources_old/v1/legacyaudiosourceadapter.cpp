...
ReadableSampleFrames LegacyAudioSourceAdapter::readSampleFramesClamped(
    ...
    if (seekFrameIndex < firstFrameIndex) {
        ...
        kLogger.info()
                ...
                << precedingFrames;
        if (precedingFrames.length() !=
                m_pImpl->readSampleFrames(precedingFrames.length(), nullptr)) {
            kLogger.warning()
    ...
    if (seekFrameIndex > firstFrameIndex) {
        ...
        if (writableSampleFrames.frameIndexRange().containsIndex(seekFrameIndex)) {
            const auto remainingFrameIndexRange =
                    IndexRange::between(seekFrameIndex,
                ...
                writableSampleFrames = WritableSampleFrames(
                        ...
                        SampleBuffer::WritableSlice(
                                writableSampleFrames.writableData(
                                        m_pOwner->getSignalInfo()
                                                .frames2samples(
                                                        unreadableFrameOffset)),
                                m_pOwner->getSignalInfo().frames2samples(
                                        remainingFrameIndexRange.length())));
            } else {
