...
SoundSource::OpenResult SoundSourceCoreAudio::tryOpen(
    ...
    case noErr:
        ...
        // See also:
        // https://developer.apple.com/documentation/audiotoolbox/audioconverterprimeinfo/1501803-leadingframes
...
SINT SoundSourceCoreAudio::seekSampleFrame(SINT frameIndex) {
    ...
    if (prefetchFrames > 0) {
        DEBUG_ASSERT(getSignalInfo().frames2samples(prefetchFrames) <=
                SINT(m_seekPrefetchBuffer.size()));
...
SINT SoundSourceCoreAudio::readSampleFrames(
    ...
    while (numFramesRead < numberOfFrames) {
        ...
        fillBufList.mBuffers[0].mDataByteSize =
                getSignalInfo().frames2samples(numFramesToRead) *
                sizeof(sampleBuffer[0]);
        fillBufList.mBuffers[0].mData =
                sampleBuffer + getSignalInfo().frames2samples(numFramesRead);
