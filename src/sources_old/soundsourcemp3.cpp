...
SoundSource::OpenResult SoundSourceMp3::tryOpen(
    ...
    do {
        ...
        if (audio::Bitrate(madHeader.bitrate).isValid()) {
            ...
            sumBitrateFrames += static_cast<quint64>(madHeader.bitrate) *
                    static_cast<quint64>(madFrameLength);
    ...
    if (differentRates > 1) {
        ...
        for (int i = 0; i < kSampleRateCount; ++i) {
            if (0 < headerPerSampleRate[i]) {
                kLogger.warning() << headerPerSampleRate[i]
                                  << "MP3 headers with sample rate"
                                  << getSampleRateByIndex(i);
            }
        }
        ...
        kLogger.warning() << "Mixxx tries to plays it with the most common "
...
void SoundSourceMp3::restartDecoding(
    ...
    // Fill input buffer
    mad_stream_buffer(&m_madStream,
            seekFrame.pInputData,
...
ReadableSampleFrames SoundSourceMp3::readSampleFramesClamped(
    ...
    while (0 < numberOfFramesRemaining) {
        ...
        if (0 >= m_madSynthCount) {
            ...
            if (madFrameChannelCount != getSignalInfo().getChannelCount()) {
                kLogger.warning()
                        << "MP3 frame header with mismatching number of "
                           "channels"
                        << madFrameChannelCount << "<>"
        ...
        if (pSampleBuffer) {
            ...
            if (madSynthChannelCount != getSignalInfo().getChannelCount()) {
                kLogger.warning()
                        << "Reading MP3 data with different number of channels"
                        << madSynthChannelCount << "<>"
    ...
    return ReadableSampleFrames(
            ...
            SampleBuffer::ReadableSlice(writableSampleFrames.writableData(),
                    std::min(writableSampleFrames.writableLength(),
