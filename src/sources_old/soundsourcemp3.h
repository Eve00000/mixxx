...
class SoundSourceMp3 final : public SoundSource {
  ...
  private:
     ...
     * decoding the whole stream. To have precise seek within a limited range
     * from the current decode position, we keep track of past decoded frame,
     * and their exact position. If a seek occurs and it is within the range of
     * frames we keep track of a precise seek occurs, otherwise an imprecise
     * seek is performed
     */
