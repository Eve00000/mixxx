...
QString SoundSource::getTypeFromFile(const QFileInfo& fileInfo) {
    ...
    if (fileSuffix == QLatin1String("flac")) {
        ...
        // Legacy FLAC files may contain an ID3 tag (written by ExactAudioCopy
        // and others) that causes these files to be identified as "audio/mpeg"
        // instead of "audio/flac". Most decoders and TagLib are able to ignore
        // and skip the non-standard ID3 data.
