...
void WTrackTableView::loadTrackModel(QAbstractItemModel* pNewModel, bool restoreState) {
    if (sDebug) {
        qDebug() << windowName
                 << " -> [loadTrackModel] toggled for model: " << pNewModel
...
void WTrackTableView::loadTrackModelInPreparationWindow(
        QAbstractItemModel* pNewModel, bool restoreState) {
    if (sDebug) {
        qDebug()
                << windowName
                << " -> [loadTrackModelInPreparationWindow] toggled for model: "
                << pNewModel << " & restoreState: " << restoreState;
        qDebug() << windowName
...
void WTrackTableView::dropEvent(QDropEvent * event) {
    ...
    if (!pTrackModel) {
        if (sDebug) {
            qDebug() << "[WTRACKTABLEVIEW] -> DROPEVENT -> pTrackModel -> ignore";
        }
        event->ignore();
        return;
    }
    ...
    if (sDebug) {
        qDebug() << "[WTRACKTABLEVIEW] -> DROPEVENT -> pTrackModel -> source" << event->source();
    }
    if (event->source() == this &&
