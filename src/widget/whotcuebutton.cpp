...
//                         .arg(context.selectString(node,
//                         QStringLiteral("Hotcue"))));
//     }
//
//     bool okay;
//     m_cueColorDimThreshold = context.selectInt(node,
//     QStringLiteral("DimBrightThreshold"), &okay); if (!okay) {
//         m_cueColorDimThreshold = kDefaultDimBrightThreshold;
//     }
//
//     m_hoverCueColor = context.selectBool(node, QStringLiteral("Hover"),
//     false);
//
//     // For dnd/swapping hotcues we use the rendered widget pixmap as dnd
//     cursor.
//     // Unfortnately the margin that constraints the bg color is not
//     considered,
//     // so we shrink the rect by custom margins.
//     okay = false;
//     int dndMargin = context.selectInt(node, QStringLiteral("DndRectMargin"),
//     &okay); if (okay && dndMargin > 0) {
//         m_dndRectMargins = QMargins(dndMargin, dndMargin, dndMargin,
//         dndMargin);
//     }
//
//     m_pCueMenuPopup = make_parented<WCueMenuPopup>(context.getConfig(),
//     this); ColorPaletteSettings colorPaletteSettings(context.getConfig());
//     auto colorPalette = colorPaletteSettings.getHotcueColorPalette();
//     m_pCueMenuPopup->setColorPalette(colorPalette);
//
//     setFocusPolicy(Qt::NoFocus);
...
//     QDomNode con = context.selectNode(node, QStringLiteral("Connection"));
//     if (!con.isNull()) {
//         SKIN_WARNING(node, context, QStringLiteral("Additional Connections
//         are not allowed"));
//     }
// }
//
...
//             // hot cue is set
//             TrackPointer pTrack =
//             PlayerInfo::instance().getTrackInfo(m_group); if (!pTrack) {
//                 return;
//             }
//
//             CuePointer pHotCue;
...
//         // i.e. these render with sharp corners and qss 'border-radius'
//         // is not visible in the drag image.
//         const QPixmap currLook =
//         grab(rect().marginsRemoved(m_dndRectMargins));
//         pDrag->setDragCursor(currLook, Qt::MoveAction);
//
//         m_dragging = true;
//         pDrag->exec();
...
//
//         // Release this button afterwards.
//         // This prevents both the preview and the pressed state from getting
//         stuck. QEvent leaveEv(QEvent::Leave); QApplication::sendEvent(this,
//         &leaveEv);
//     }
// }
//
...
//     key.group = m_group;
//     // Add one to hotcue so that we don't have a hotcue_0
//     key.item = QStringLiteral("hotcue_") + QString::number(m_hotcue + 1) +
//     QChar('_') + name; return key;
// }
//
// void WHotcueButton::slotColorChanged(double color) {
//     VERIFY_OR_DEBUG_ASSERT(color >= 0 && color <= 0xFFFFFF) {
...
//     }
//     QColor cueColor = QColor::fromRgb(static_cast<QRgb>(color));
//     m_bCueColorDimmed = Color::isDimColorCustom(cueColor,
//     m_cueColorDimThreshold);
//
//     QString style =
//             QStringLiteral(
//                     "WWidget[displayValue=\"1\"], "
