...
inline WLibraryPreparationWindow* findLibraryPreparationWindowWidgetParent(QWidget* pWidget) {
    while (pWidget) {
        WLibraryPreparationWindow* pPreparationWindow =
                qobject_cast<WLibraryPreparationWindow*>(pWidget);
        if (pPreparationWindow) {
            return pPreparationWindow;
        }
...
OverviewDelegate::OverviewDelegate(QTableView* pTableView)
    ...
    WLibraryPreparationWindow* pPreparationWindow =
            findLibraryPreparationWindowWidgetParent(pTableView);
    if (pPreparationWindow) {
        m_signalColors = pPreparationWindow->getOverviewSignalColors();
    }
