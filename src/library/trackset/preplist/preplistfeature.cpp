...
#include "moc_preplistfeature.cpp"
...
PrepListFeature::PrepListFeature(
        Library* pLibrary,
...
QVariant PrepListFeature::title() {
    return tr("Preparation");
}

void PrepListFeature::bindLibraryWidget(
    ...
    connect(&PlayerInfo::instance(),
            ...
            this,
            &PrepListFeature::slotAddLoadedTrackToPreparation);
    m_pLibraryWidget = QPointer(pLibraryWidget);
}

void PrepListFeature::deleteAllUnlockedPlaylistsWithFewerTracks() {
    ...
    m_playlistDao.deleteAllUnlockedPlaylistsWithFewerTracks(PlaylistDAO::PLHT_PREPLIST,
            minTrackCount);
    transaction.commit();
}

void PrepListFeature::slotDeletePlaylist() {
...
bool PrepListFeature::dropAcceptChild(
        const QModelIndex& index, const QList<QUrl>& urls, QObject* pSource) {
    qDebug() << "[PrepListFeature] -> dropAcceptChild triggered";
...
bool PrepListFeature::dragMoveAcceptChild(const QModelIndex& index, const QUrl& url) {
    int playlistId = playlistIdFromIndex(index);
...
void PrepListFeature::onRightClick(const QPoint& globalPos) {
    Q_UNUSED(globalPos);
    m_lastRightClickedIndex = QModelIndex();
}

void PrepListFeature::onRightClickChild(const QPoint& globalPos, const QModelIndex& index) {
...
QModelIndex PrepListFeature::constructChildModel(int selectedId) {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  constructChildModel() selected:" << selectedId;
    }
    ...
    QString queryString = QStringLiteral(
            ...
            "  GROUP BY Playlists.id")
                                  .arg(m_countsDurationTableName,
                                          ...
                                          QString::number(PlaylistDAO::PLHT_PREPLIST));
    ;
    queryString.append(
    ...
    for (int row = 0; row < playlistTableModel.rowCount(); ++row) {
        ...
        if (hiddenType == PlaylistDAO::PLHT_PREPLIST) {
...
void PrepListFeature::decorateChild(TreeItem* item, int playlistId) {
    if (playlistId == m_currentPlaylistId) {
...
void PrepListFeature::slotGetNewPlaylist() {
    if (sDebug) {
        qDebug() << "[PrepListFeature] -> slotGetNewPlaylist() successfully triggered !";
    }
    ...
    if (sDebug) {
        qDebug() << "[PrepListFeature] -> Creating session preparation "
                    "playlist name:"
                 << preparation_name;
    }
    m_currentPlaylistId = m_playlistDao.createPlaylist(
            preparation_name, PlaylistDAO::PLHT_PREPLIST);

    if (m_currentPlaylistId == kInvalidPlaylistId) {
        if (sDebug) {
            qDebug() << "[PrepListFeature] -> Setlog playlist Creation Failed";
            qDebug() << "[PrepListFeature] -> An unknown error occurred while creating playlist: "
                     << preparation_name;
        }
    } else {
        m_recentTracks.clear();
        // As the PrepListFeature is constructed before the skin & prepwin are completed,
        // we need to store the newest created preparationlistID in a temp, pending to be sent
        // to the PrepWin.
        m_pendingPlaylistId = m_currentPlaylistId;
        m_pendingPlaylistName = preparation_name;
    }
...
void PrepListFeature::onPreparationWindowReady() {
    if (m_pendingPlaylistId != kInvalidPlaylistId) {
        qDebug() << "[PrepListFeature] -> Delayed emitting the new "
                    "Preparationlist to Prepwin when the widget (skin) is "
                    "ready.";
        qDebug() << "[PrepListFeature] -> Newest PreparationList Id: " << m_pendingPlaylistId;
        qDebug() << "[PrepListFeature] -> Newest PreparationList Name: " << m_pendingPlaylistName;
...
void PrepListFeature::slotJoinWithPrevious() {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotJoinWithPrevious() row:"
    ...
    if (locked) {
        qDebug() << "[PrepListFeature] -> Aborting playlist join because playlist"
    ...
    int previousPlaylistId = m_playlistDao.getPreviousPlaylist(
            clickedPlaylistId, PlaylistDAO::PLHT_PREPLIST);
    ...
    qDebug() << "[PrepListFeature] -> slotJoinWithPrevious() current:"
             << clickedPlaylistId
...
void PrepListFeature::slotMarkAllTracksPlayed() {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotMarkAllTracksPlayed()";
    }
    if (!m_lastRightClickedIndex.isValid()) {
        return;
    }
...
void PrepListFeature::slotLockAllChildPlaylists() {
    lockOrUnlockAllChildPlaylists(true);
}

void PrepListFeature::slotUnlockAllChildPlaylists() {
    lockOrUnlockAllChildPlaylists(false);
}

void PrepListFeature::lockOrUnlockAllChildPlaylists(bool lock) {
    if (!m_lastRightClickedIndex.isValid()) {
        return;
    }
    if (lock) {
        qWarning() << "[PrepListFeature] -> lock all child playlists of"
                   << m_lastRightClickedIndex.data().toString();
    } else {
        qWarning() << "[PrepListFeature] -> unlock all child playlists of"
                   << m_lastRightClickedIndex.data().toString();
    }
...
void PrepListFeature::slotDeleteAllUnlockedChildPlaylists() {
    if (!m_lastRightClickedIndex.isValid()) {
        return;
    }
    TreeItem* item = static_cast<TreeItem*>(m_lastRightClickedIndex.internalPointer());
    ...
    if (sDebug) {
        qDebug() << "[PrepListFeature] -> deleting all unlocked playlists of" << year;
    }
    m_playlistDao.deleteUnlockedPlaylists(std::move(ids));
}

// pLoadedTrack
void PrepListFeature::slotAddLoadedTrackToPreparation(const QString& group,
    ...
    if (loadedTrack) {
        ...
        if (sDebug) {
            qDebug() << "[PrepListFeature] -> Received Track ID" << newLoadedTrack;
        }
    } else {
        if (sDebug) {
            qDebug() << "[PrepListFeature] -> Received null track!";
        }
        return;
    }
    ...
    if (sDebug) {
        qDebug() << "PrepListFeature: Loaded Track ID received: " << newLoadedTrackId;
    }
    ...
    if (m_pConfig->getValue(ConfigKey("[Library]", "PreparationListsAddAllLoadedTracks"), true)) {
        if (m_pPlaylistTableModel->getPlaylist() == m_currentPlaylistId) {
            if (sDebug) {
                qDebug() << "PrepListFeature: Loaded Preparation PlayListId "
...
void PrepListFeature::slotPlaylistTableChanged(int playlistId) {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotPlaylistTableChanged() id:" << playlistId;
    }
    PlaylistDAO::HiddenType type = m_playlistDao.getHiddenType(playlistId);
    if (type != PlaylistDAO::PLHT_PREPLIST &&
...
void PrepListFeature::slotPlaylistContentOrLockChanged(const QSet<int>& playlistIds) {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotPlaylistContentOrLockChanged() for"
                 << playlistIds.count() << "playlist(s)";
    }
    QSet<int> idsToBeUpdated;
    for (const auto playlistId : std::as_const(playlistIds)) {
        if (m_playlistDao.getHiddenType(playlistId) == PlaylistDAO::PLHT_PREPLIST) {
...
void PrepListFeature::slotPlaylistTableRenamed(int playlistId, const QString& newName) {
    Q_UNUSED(newName);
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotPlaylistTableRenamed() Id:" << playlistId;
    }
    if (m_playlistDao.getHiddenType(playlistId) == PlaylistDAO::PLHT_PREPLIST) {
...
void PrepListFeature::slotShowInLibraryWindow() {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotShowInLibraryWindow()";
    }
    int playlistId = playlistIdFromIndex(m_lastRightClickedIndex);
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  slotShowInLibraryWindow() playlistId: " << playlistId;
    }
    if (playlistId == kInvalidPlaylistId) {
...
void PrepListFeature::activateChild(const QModelIndex& index) {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  activateChild()" << index;
    }
    int playlistId = playlistIdFromIndex(index);
    if (playlistId == kInvalidPlaylistId) {
        // may happen during initialization
...
void PrepListFeature::activatePlaylist(int playlistId) {
    if (sDebug) {
        qDebug() << "[PrepListFeature] ->  activatePlaylist()" << playlistId;
    }
    if (playlistId == kInvalidPlaylistId) {
        return;
    }
...
QString PrepListFeature::getRootViewHtml() const {
