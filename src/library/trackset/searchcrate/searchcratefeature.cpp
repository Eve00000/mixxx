...
SearchCrateFeature::SearchCrateFeature(Library* pLibrary,
...
// bool SearchCrateFeature::dragMoveAcceptChild(const QModelIndex& index, const QList<QUrl>& urls) {
// SearchCrateId searchCrateId(searchCrateIdFromIndex(index));
// if (!searchCrateId.isValid()) {
// return false;
// }
...
QModelIndex SearchCrateFeature::rebuildChildModel(SearchCrateId selectedSearchCrateId) {
    if (sDebugSearchCrateFeature) {
        qDebug() << "[SEARCHCRATESFEATURE] [RebuildChildModel]" << selectedSearchCrateId;
        ;
    }
...
//     SearchCrateId searchCrateId = searchCrateIdFromIndex(m_lastRightClickedIndex);
//     SearchCrate crate;
//     if (m_pTrackCollection->searchCrates().readSearchCrateById(searchCrateId, &crate)) {
//         qDebug() << "Importing playlist file" << playlistFile << "into crate"
//                  << searchCrateId << crate;
//     } else {
//         qDebug() << "Importing playlist file" << playlistFile << "into crate"
//                  << searchCrateId << crate << "failed!";
//         return;
//     }
//
...
// void SearchCrateFeature::slotImportPlaylistFile(const QString& playlistFile,
// SearchCrateId searchCrateId) {
//     SearchCrate searchCrate;
//     if
//     (!m_pTrackCollection->searchCrates().readSearchCrateById(searchCrateId,
//     &searchCrate)) {
//         return;
//     }
//     if (searchCrate.isLocked()) {
...
//         another
//         // crate selected which is not the crate that received the
//         right-click. std::unique_ptr<SearchCrateTableModel>
