...
void PreparationFeature::slotGetNewPlaylist() {
    ...
    if (sDebug) {
        qDebug() << "[PreparationFeature] -> Creating session preparation "
                    "playlist name:"
                 << preparation_name;
    }
    ...
    if (m_currentPlaylistId == kInvalidPlaylistId) {
        if (sDebug) {
            ...
            qDebug() << "[PreparationFeature] -> An unknown error occurred "
                        "while creating playlist: "
                     << preparation_name;
        }
    } else {
...
void PreparationFeature::onPreparationWindowReady() {
    if (m_pendingPlaylistId != kInvalidPlaylistId) {
        ...
        qDebug() << "[PreparationFeature] -> Newest PreparationList Name: "
                 << m_pendingPlaylistName;
...
void PreparationFeature::lockOrUnlockAllChildPlaylists(bool lock) {
    ...
    if (lock) {
        qWarning() << "[PreparationFeature] -> lock all child playlists of"
                   << m_lastRightClickedIndex.data().toString();
    } else {
        qWarning() << "[PreparationFeature] -> unlock all child playlists of"
                   << m_lastRightClickedIndex.data().toString();
    }
