...
void EngineBufferScaleRubberBand::setScaleParameters(double base_rate,
    ...
    // 2. KEEP SPLIT WORKERS PERFECTLY IN SYNC
    if (m_isStemMode && m_premixWorker && m_stemsWorker) {
        if (pitchScale > 0) {
            m_premixWorker->setPitchScale(pitchScale);
            m_stemsWorker->setPitchScale(pitchScale);
        }
        if (timeRatioInverse > 0) {
            m_premixWorker->setTimeRatio(1.0 / timeRatioInverse);
            m_stemsWorker->setTimeRatio(1.0 / timeRatioInverse);
        }
    }
}
...
SINT EngineBufferScaleRubberBand::retrieveAndDeinterleave(
    ...
    // === FIX: UNCONDITIONALLY SANITY CHECK AND RESIZE ALL STRIPS TO PREVENT QLIST BLOWUPS ===
    if (m_premixBuffers.size() != 2) {
    ...
    if (m_stemsBuffers.size() != 8) {
        m_stemsBuffers.resize(8);
        m_stemsPtrs.resize(8);
    }
    for (int ch = 0; ch < 8; ch++) {
        if (static_cast<SINT>(m_stemsBuffers[ch].size()) < framesToRetrieve) {
            m_stemsBuffers[ch] = mixxx::SampleBuffer(framesToRetrieve);
        }
        m_stemsPtrs[ch] = m_stemsBuffers[ch].data();
    }
...
void EngineBufferScaleRubberBand::initStemWorkers() {
    ...
    // Fix: Match main stretcher initial time ratios properly
    double timeRatioInverse = m_dBaseRate * m_dTempoRatio;
    if (timeRatioInverse > 0) {
        m_premixWorker->setTimeRatio(1.0 / timeRatioInverse);
        m_stemsWorker->setTimeRatio(1.0 / timeRatioInverse);
    }
    
    double pitchScale = fabs(m_dBaseRate * m_dPitchRatio);
    if (pitchScale > 0) {
        m_premixWorker->setPitchScale(pitchScale);
        m_stemsWorker->setPitchScale(pitchScale);
    }
