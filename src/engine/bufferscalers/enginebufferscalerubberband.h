...
class EngineBufferScaleRubberBand final : public EngineBufferScale {
  ...
  private:
    ...
    // For STEM + PreMix mode: two synced workers
    std::unique_ptr<RubberBandWrapper> m_premixWorker; // 2 channels
    std::unique_ptr<RubberBandWrapper> m_stemsWorker;  // 8 channels
    ...
    bool m_isStemMode = false;
    SINT m_syncFrameCounter = 0;

    std::vector<mixxx::SampleBuffer> m_premixBuffers;
    std::vector<float*> m_premixPtrs;
    std::vector<mixxx::SampleBuffer> m_stemsBuffers;
    std::vector<float*> m_stemsPtrs;
};
