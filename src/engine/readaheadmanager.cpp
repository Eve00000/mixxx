...
//         SINT requested_samples,
//         mixxx::audio::ChannelCount channelCount) {
//     // qDebug() << "getNextSamples:" << m_currentPosition <<
//     requested_samples;
//
//     int modSamples = requested_samples % channelCount;
//     if (modSamples != 0) {
//         qDebug() << "ERROR: Non-aligned requested_samples to
//         ReadAheadManager::getNextSamples"; requested_samples -= modSamples;
//     }
//     bool in_reverse = dRate < 0;
//
//     mixxx::audio::FramePos targetPosition;
...
//     loopTriggerPosition.toSamplePosMaybeInvalid(channelCount); const double
//     target = targetPosition.toSamplePosMaybeInvalid(channelCount);
//
...
//             // clamp requested samples from the caller to the loop trigger
//             point if (preloop_samples <= requested_samples) {
//                 reachedTrigger = true;
...
//         ReadAheadManager::getNextSamples. Ignoring read"; return 0;
//     }
//
//     SINT start_sample = SampleUtil::roundPlayPosToFrameStart(
//             m_currentPosition, channelCount);
...
//
//     // Increment or decrement current read-ahead position
//     // Mixing int and double here is desired, because the fractional frame
//     should
//     // be resist
//     if (in_reverse) {
//         addReadLogEntry(m_currentPosition, m_currentPosition -
//         samples_from_reader); m_currentPosition -= samples_from_reader;
//     } else {
//         addReadLogEntry(m_currentPosition, m_currentPosition +
//         samples_from_reader); m_currentPosition += samples_from_reader;
//     }
...
//         DEBUG_ASSERT(target != kNoTrigger);
//         if (m_pRateControl) {
//             m_pRateControl->notifyWrapAround(loopTriggerPosition,
//             targetPosition);
//         }
//         // TODO probably also useful for hotcue_X_indicator in
//         CueControl::updateIndicators()
//
//         // Jump to other end of loop or track.
//         m_currentPosition = target;
...
//             // start the loop later accordingly to be sure the loop length is
//             as desired
//             // e.g. exactly one bar.
//             m_currentPosition += overshoot;
//
...
//         }
//
//         // start reading before the loop start point, to crossfade these
//         samples
//         // with the samples we need to the loop end
...
//             // we start in the pre-role without suitable samples for
//             crossfading crossFadeStart = -loop_read_position;
//             crossFadeSamples -= crossFadeStart;
//         } else {
//             int trackSamples = static_cast<int>(
//                     m_pLoopingControl->getTrackFrame().toSamplePos(
...
//             }
//
//             // do crossfade from the current buffer into the new loop
//             beginning if (samples_from_reader != 0) { // avoid division by
//             zero
//                 SampleUtil::linearCrossfadeBuffersOut(
//                         pOutput +
//                         SampleUtil::ceilPlayPosToFrameStart(crossFadeStart,
//                         channelCount), m_pCrossFadeBuffer, crossFadeSamples,
//                         channelCount);
//             }
//         } else {
//             // No samples for crossfading, ramp to zero
...
//     m_readAheadLog.clear();
//
//     // TODO(XXX) notifySeek on the engine controls. EngineBuffer currently
//     does
//     // a fine job of this so it isn't really necessary but eventually I think
//     // RAMAN should do this job. rryan 11/2011
//
...
//     Hint current_position;
//
//     // SoundTouch can read up to 2 chunks ahead. Always keep 2 chunks ahead
//     in
//     // cache.
//     SINT frameCountToCache = 2 * CachingReaderChunk::kFrames;
//     current_position.frameCount = frameCountToCache;
...
//                     numConsumedFrames * channelCount,
//                     channelCount);
//     return mixxx::audio::FramePos::fromSamplePos(positionSamples,
//     channelCount);
// }
