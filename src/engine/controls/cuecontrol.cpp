...
///// This is the position of a fresh loaded tack without any seek
// constexpr int kNoHotCueNumber = 0;
///// Used for a common tracking of the previewing Hotcue in
/// m_currentlyPreviewingIndex / constexpr int kMainCueIndex = NUM_HOT_CUES;
...
//     if (hotcueIndex >= mixxx::kFirstHotCueIndex) {
//         DEBUG_ASSERT(hotcueIndex != Cue::kNoHotCue);
//         return (hotcueIndex - mixxx::kFirstHotCueIndex) + 1; // to 1-based
//         numbering
//     } else {
//         DEBUG_ASSERT(hotcueIndex == Cue::kNoHotCue);
//         return kNoHotCueNumber;
...
//     if (hotcueNumber >= 1) {
//         DEBUG_ASSERT(hotcueNumber != kNoHotCueNumber);
//         return mixxx::kFirstHotCueIndex + (hotcueNumber - 1); // from 1-based
//         numbering
//     } else {
//         DEBUG_ASSERT(hotcueNumber == kNoHotCueNumber);
//         return Cue::kNoHotCue;
...
//         const Hint cueHint = {
//                 /*.frame
//                 =*/static_cast<SINT>(frame.toLowerFrameBoundary().value()),
//                 /*.frameCount =*/Hint::kFrameCountForward,
//                 /*.type =*/type};
//         pHintList->append(cueHint);
//     }
// }
//
// void appendCueHint(gsl::not_null<HintVector*> pHintList, const double
// playPos, Hint::Type type) {
//     const auto frame =
//     mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(playPos);
//     appendCueHint(pHintList, frame, type);
// }
//
// } // namespace
...
//     connectControls();
//
//     m_pTrackSamples = ControlObject::getControl(ConfigKey(group,
//     "track_samples"));
//
//     m_pQuantizeEnabled = ControlObject::getControl(ConfigKey(group,
//     "quantize")); connect(m_pQuantizeEnabled,
//             &ControlObject::valueChanged,
//             this,
//             &CueControl::quantizeChanged,
//             Qt::DirectConnection);
//
//     m_pClosestBeat = ControlObject::getControl(ConfigKey(group,
//     "beat_closest")); m_pLoopStartPosition =
//     make_parented<ControlProxy>(group, "loop_start_position", this);
//     m_pLoopEndPosition = make_parented<ControlProxy>(group,
//     "loop_end_position", this); m_pLoopEnabled =
//     make_parented<ControlProxy>(group, "loop_enabled", this);
//     m_pBeatLoopActivate = make_parented<ControlProxy>(group,
//     "beatloop_activate", this); m_pBeatLoopSize =
//     make_parented<ControlProxy>(group, "beatloop_size", this);
//
...
//
//     m_pSortHotcuesByPos =
//     std::make_unique<ControlPushButton>(ConfigKey(group, "sort_hotcues"));
//     connect(m_pSortHotcuesByPos.get(),
//             &ControlObject::valueChanged,
//             this,
//             &CueControl::setHotcueIndicesSortedByPosition,
...
//
// void CueControl::createControls() {
//     m_pCueSet = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "cue_set"));
//     m_pCueSet->setButtonMode(mixxx::control::ButtonMode::Trigger);
//     m_pCueClear = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "cue_clear"));
//     m_pCueClear->setButtonMode(mixxx::control::ButtonMode::Trigger);
//     m_pCueGoto = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "cue_goto")); m_pCueGotoAndPlay =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "cue_gotoandplay")); m_pCuePlay =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group, "cue_play"));
//     m_pCueGotoAndStop =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "cue_gotoandstop")); m_pCuePreview =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group, "cue_preview"));
//     m_pCueCDJ = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "cue_cdj")); m_pCueDefault =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group, "cue_default"));
//     m_pPlayStutter = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "play_stutter"));
//
//     m_pPlayLatched = std::make_unique<ControlObject>(ConfigKey(m_group,
//     "play_latched")); m_pPlayLatched->setReadOnly();
//
//     m_pCueIndicator = std::make_unique<ControlIndicator>(ConfigKey(m_group,
//     "cue_indicator")); m_pPlayIndicator =
//     std::make_unique<ControlIndicator>(ConfigKey(m_group, "play_indicator"));
//
//     m_pIntroStartPosition = std::make_unique<ControlObject>(
//             ConfigKey(m_group, "intro_start_position"));
//     m_pIntroStartPosition->set(Cue::kNoPosition);
...
//             ConfigKey(m_group, "intro_start_enabled"));
//     m_pIntroStartEnabled->setReadOnly();
//     m_pIntroStartSet = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "intro_start_set")); m_pIntroStartClear =
//     std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "intro_start_clear"));
//     m_pIntroStartActivate = std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "intro_start_activate"));
//     m_pIntroEndPosition = std::make_unique<ControlObject>(ConfigKey(m_group,
//     "intro_end_position")); m_pIntroEndPosition->set(Cue::kNoPosition);
//     m_pIntroEndEnabled = std::make_unique<ControlObject>(ConfigKey(m_group,
//     "intro_end_enabled")); m_pIntroEndEnabled->setReadOnly(); m_pIntroEndSet
//     = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "intro_end_set")); m_pIntroEndClear =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "intro_end_clear")); m_pIntroEndActivate =
//     std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "intro_end_activate"));
//
//     m_pOutroStartPosition = std::make_unique<ControlObject>(
...
//             ConfigKey(m_group, "outro_start_enabled"));
//     m_pOutroStartEnabled->setReadOnly();
//     m_pOutroStartSet = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "outro_start_set")); m_pOutroStartClear =
//     std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "outro_start_clear"));
//     m_pOutroStartActivate = std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "outro_start_activate"));
//     m_pOutroEndPosition = std::make_unique<ControlObject>(ConfigKey(m_group,
//     "outro_end_position")); m_pOutroEndPosition->set(Cue::kNoPosition);
//     m_pOutroEndEnabled = std::make_unique<ControlObject>(ConfigKey(m_group,
//     "outro_end_enabled")); m_pOutroEndEnabled->setReadOnly(); m_pOutroEndSet
//     = std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "outro_end_set")); m_pOutroEndClear =
//     std::make_unique<ControlPushButton>(ConfigKey(m_group,
//     "outro_end_clear")); m_pOutroEndActivate =
//     std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "outro_end_activate"));
//
//     m_pVinylControlEnabled = std::make_unique<ControlProxy>(m_group,
//     "vinylcontrol_enabled"); m_pVinylControlMode =
//     std::make_unique<ControlProxy>(m_group, "vinylcontrol_mode");
//
//     m_pHotcueFocus = std::make_unique<ControlObject>(ConfigKey(m_group,
//     "hotcue_focus")); setHotcueFocusIndex(Cue::kNoHotCue);
//     m_pHotcueFocusColorPrev = std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "hotcue_focus_color_prev"));
//     m_pHotcueFocusColorNext = std::make_unique<ControlPushButton>(
//             ConfigKey(m_group, "hotcue_focus_color_next"));
...
// void CueControl::passthroughChanged(double enabled) {
//     if (enabled > 0) {
//         // If passthrough was enabled seeking and playing is prohibited, and
//         the
//         // waveform and overview are blocked.
//         // Disconnect all cue controls to prevent cue changes without UI
//         feedback. disconnectControls();
//     } else {
//         // Reconnect all controls when deck returns to regular mode.
//         connectControls();
//     }
...
//
//// This is called from the EngineWokerThread and ends with the initial seek
//// via seekOnLoad(). There is the theoretical and pending issue of a delayed
/// control / command intended for the old track that might be performed instead.
// void CueControl::trackLoaded(TrackPointer pNewTrack) {
//     auto lock = lockMutex(&m_trackMutex);
//     if (m_pLoadedTrack) {
//         disconnect(m_pLoadedTrack.get(), nullptr, this, nullptr);
...
//     switch (seekOnLoadMode) {
//     case SeekOnLoadMode::Beginning:
//         // This allows users to load tracks and have the needle-drop be
//         maintained. if (!(m_pVinylControlEnabled->toBool() &&
...
//         // value in CO will be quantized if quantization is enabled
//         // while value in cue point list will never be quantized.
//         // This prevents jumps when track analysis finishes while
//         quantization is enabled. const auto mainCuePosition =
//                 mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(
//                         m_pCuePoint->get());
//         if (mainCuePosition.isValid()) {
//             seekOnLoad(mainCuePosition);
...
//             } else {
//                 // If the old hotcue is the same, then we only need to update
//                 Cue::StartAndEndPositions pos =
//                 pCue->getStartAndEndPosition();
//                 pControl->setPosition(pos.startPosition);
//                 pControl->setEndPosition(pos.endPosition);
//                 pControl->setColor(pCue->getColor());
//                 pControl->setType(pCue->getType());
...
//         quantizeCuePoint(pIntroCue->getPosition()); const auto endPosition =
...
//     if (pOutroCue) {
//         const auto startPosition =
//         quantizeCuePoint(pOutroCue->getPosition()); const auto endPosition =
//         quantizeCuePoint(pOutroCue->getEndPosition());
//
//         m_pOutroStartPosition->set(startPosition.toEngineSamplePosMaybeInvalid());
//         m_pOutroStartEnabled->forceSet(startPosition.isValid());
//         m_pOutroEndPosition->set(endPosition.toEngineSamplePosMaybeInvalid());
...
//             // This allows to set a hotuce inside an active, saved loop with
//             // 'hotcue_X_activate'.
//             auto* pSavedLoopControl =
//             m_pCurrentSavedLoopControl.loadAcquire(); if (pSavedLoopControl
//             &&
//                     pSavedLoopControl->getPosition() ==
//                             mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(
//                                     m_pLoopStartPosition->get()) &&
...
//     ControlFlag::AllowMissingOrInvalid); if (proxyStem.get() > 1) {
//         // qDebug() << "[CUECONTROL] -> stem -> proxyStem > 1";
//         const QString groupBaseName = getGroup().remove("[").remove("]");
//         const QString stemGroups[] = {
//                 QString("[%1_Stem1]").arg(groupBaseName),
...
//         // get the mute multiplier
//         auto getMuteMultiplier = [](const QString& group) -> int {
//             PollingControlProxy proxyMute(group, "mute",
//             ControlFlag::AllowMissingOrInvalid); return
//             static_cast<bool>(proxyMute.get()) ? -1 : 1;
//         };
//
//         // get the volume value adjusted by the mute multiplier
//         auto getVolume = [](const QString& group, int muteMultiplier) ->
//         double {
//             PollingControlProxy proxyVolume(group, "volume",
//             ControlFlag::AllowMissingOrInvalid); return
//             static_cast<double>(proxyVolume.get()) * muteMultiplier;
//         };
//
//         // calc stem volume values
//         pass2CueCreationStem1Vol = getVolume(stemGroups[0],
//         getMuteMultiplier(stemGroups[0])); pass2CueCreationStem2Vol =
//         getVolume(stemGroups[1], getMuteMultiplier(stemGroups[1]));
//         pass2CueCreationStem3Vol = getVolume(stemGroups[2],
//         getMuteMultiplier(stemGroups[2])); pass2CueCreationStem4Vol =
//         getVolume(stemGroups[3], getMuteMultiplier(stemGroups[3]));
//     } else {
//         qDebug() << "[CUECONTROL] -> stem -> proxyStem = 1";
//         pass2CueCreationStem1Vol = 1.0;
//         pass2CueCreationStem2Vol = 1.0;
...
//             if (cueStartPosition.isValid()) {
//                 cueEndPosition =
//                 pBeats->findNBeatsFromPosition(cueStartPosition,
//                 beatloopSize);
//             }
//         }
//         cueType = mixxx::CueType::Loop;
...
//         if (getConfig()->getValue(autoLoopColorsKey, false)) {
//             color =
//             m_colorPaletteSettings.getHotcueColorPalette().colorForHotcueIndex(hotcueIndex);
//         } else {
//             color = colorFromConfig(ConfigKey("[Controls]",
...
//     case mixxx::CueType::Loop: {
//         // The hotcue_X_cueloop CO was invoked for a saved loop, set it as
//         // active the first time this happens and toggle the loop_enabled
//         state
//         // on subsequent invocations.
//         if (m_pCurrentSavedLoopControl != pControl) {
//             setCurrentSavedLoopControlAndActivate(pControl);
//         } else {
//             bool loopActive = pControl->getStatus() ==
//             HotcueControl::Status::Active; Cue::StartAndEndPositions pos =
//             pCue->getStartAndEndPosition(); setLoop(pos.startPosition,
//             pos.endPosition, !loopActive);
//         }
//     } break;
//     case mixxx::CueType::HotCue: {
//         // The hotcue_X_cueloop CO was invoked for a hotcue. In that case,
//         // create a beatloop starting at the hotcue position. This is useful
//         for
//         // mapping the CUE LOOP mode labeled on some controllers.
//         setCurrentSavedLoopControlAndActivate(nullptr);
//         const mixxx::audio::FramePos startPosition = pCue->getPosition();
...
//                 getGroup().remove('[').remove(']'); const
//                 std::vector<QString> stemGroups = {
//                         QString("[%1_Stem1]").arg(groupBaseName),
//                         QString("[%1_Stem2]").arg(groupBaseName),
//                         QString("[%1_Stem3]").arg(groupBaseName),
//                         QString("[%1_Stem4]").arg(groupBaseName)};
...
//             }
//             // EveCue-Loop
//             if (m_pPlay->toBool() && m_currentlyPreviewingIndex ==
//             Cue::kNoHotCue) {
//                 // playing by Play button
//                 switch (pCue->getType()) {
//                 case mixxx::CueType::HotCue:
...
//                         Cue::StartAndEndPositions pos =
//                         pCue->getStartAndEndPosition();
//                         setLoop(pos.startPosition, pos.endPosition,
//                         !loopActive);
//                     }
//                     break;
//                 default:
...
//             pControl->getPreviewingPosition(); mixxx::CueType type =
//             pControl->getPreviewingType(); if (type !=
//             mixxx::CueType::Invalid && position.isValid()) {
//                 updateCurrentlyPreviewingIndex(index);
//                 m_bypassCueSetByPlay = true;
//                 if (type == mixxx::CueType::Loop) {
//                     setCurrentSavedLoopControlAndActivate(pControl);
...
//         // This is a release of a previewing hotcue
//         const mixxx::audio::FramePos position =
//         pControl->getPreviewingPosition();
//         updateCurrentlyPreviewingIndex(Cue::kNoHotCue);
//         m_pPlay->set(0.0);
//         if (position.isValid()) {
//             seekExact(position);
...
// void CueControl::updateCurrentlyPreviewingIndex(int hotcueIndex) {
//     int oldPreviewingIndex =
//     m_currentlyPreviewingIndex.fetchAndStoreRelease(hotcueIndex); if
//     (oldPreviewingIndex >= 0 && oldPreviewingIndex < m_iNumHotCues) {
//         // We where already in previewing state, clean up ..
//         HotcueControl* pLastControl =
//         m_hotcueControls.at(oldPreviewingIndex); mixxx::CueType lastType =
//         pLastControl->getPreviewingType(); if (lastType ==
//         mixxx::CueType::Loop) {
//             m_pLoopEnabled->set(0);
//         }
//         CuePointer pLastCue(pLastControl->getCue());
...
//     // Setting the position to Cue::kNoPosition is the same as calling
//     hotcue_x_clear if (!newPosition.isValid()) {
//         detachCue(pControl);
//         return;
//     }
//
...
//     }
//
//     if (pCue->getType() == mixxx::CueType::Loop && newPosition >=
//     pCue->getEndPosition()) {
//         return;
//     }
//     pCue->setStartPosition(newPosition);
...
//
//     // this is called from the engine thread
//     // it is no locking required, because m_hotcueControl is filled during
//     the
//     // constructor and getPosition()->get() is a ControlObject
//     for (const auto& pControl : std::as_const(m_hotcueControls)) {
//         appendCueHint(pHintList, pControl->getPosition(),
//         Hint::Type::HotCue);
//     }
//
//     appendCueHint(pHintList, m_n60dBSoundStartPosition.getValue(),
//     Hint::Type::FirstSound); appendCueHint(pHintList,
//     m_pIntroStartPosition->get(), Hint::Type::IntroStart);
//     appendCueHint(pHintList, m_pIntroEndPosition->get(),
//     Hint::Type::IntroEnd); appendCueHint(pHintList,
//     m_pOutroStartPosition->get(), Hint::Type::OutroStart);
// }
//
//// Moves the cue point to current position or to closest beat in case
//// quantize is enabled
...
//                     m_pCuePoint->get());
//
//     // Note: We do not mess with play here, we continue playing or
//     previewing.
//
//     // Need to unlock before emitting any signals to prevent deadlock.
//     lock.unlock();
...
// void CueControl::cueCDJ(double value) {
//     // This is how Pioneer cue buttons work:
//     // If pressed while freely playing (i.e. playing and platter NOT being
//     touched), stop playback and go to cue.
//     // If pressed while NOT freely playing (i.e. stopped or playing but
//     platter IS being touched), set new cue point.
//     // If pressed while stopped and at cue, play while pressed.
//     // If play is pressed while holding cue, the deck is now playing.
//     (Handled in playFromCuePreview().)
//
//     const auto freely_playing =
//             m_pPlay->toBool() && !getEngineBuffer()->getScratching();
//     TrackAt trackAt = getTrackAt();
...
// void CueControl::cuePlay(double value) {
//     // This is how CUP button works:
//     // If freely playing (i.e. playing and platter NOT being touched), press
//     to go to cue and stop.
//     // If not freely playing (i.e. stopped or platter IS being touched),
//     press to go to cue and stop.
//     // On release, start playing from cue point.
//
//     const auto freely_playing =
//             m_pPlay->toBool() && !getEngineBuffer()->getScratching();
...
//     if (introEnd.isValid() && position >= introEnd) {
//         qWarning()
//                 << "Trying to place intro start cue on or after intro end
//                 cue.";
//         return;
//     }
//     if (outroStart.isValid() && position >= outroStart) {
//         qWarning() << "Trying to place intro start cue on or after outro
//         start "
//                       "cue.";
//         return;
//     }
//     if (outroEnd.isValid() && position >= outroEnd) {
//         qWarning()
//                 << "Trying to place intro start cue on or after outro end
//                 cue.";
//         return;
//     }
//
...
//     if (outroStart.isValid() && position >= outroStart) {
//         qWarning()
//                 << "Trying to place intro end cue on or after outro start
//                 cue.";
//         return;
//     }
//     if (outroEnd.isValid() && position >= outroEnd) {
//         qWarning()
//                 << "Trying to place intro end cue on or after outro end
//                 cue.";
//         return;
//     }
//
...
//     if (outroEnd.isValid() && position >= outroEnd) {
//         qWarning()
//                 << "Trying to place outro start cue on or after outro end
//                 cue.";
//         return;
//     }
//
...
//     if (introEnd.isValid() && position <= introEnd) {
//         qWarning()
//                 << "Trying to place outro end cue on or before intro end
//                 cue.";
//         return;
//     }
//     if (outroStart.isValid() && position <= outroStart) {
...
//     //         << m_iCurrentlyPreviewingHotcues << m_bPreviewing;
//     CueMode cueMode =
//     static_cast<CueMode>(static_cast<int>(m_pCueMode->get())); if ((cueMode
//     == CueMode::Denon || cueMode == CueMode::Numark) &&
//             newPlay && !oldPlay && playPossible &&
//             !m_bypassCueSetByPlay) {
//         // in Denon mode each play from pause moves the cue point
//         // if not previewing
...
//     m_bypassCueSetByPlay = false;
//
//     // when previewing, "play" was set by cue button, a following toggle
//     request
//     // (play = 0.0) is used for latching play.
//     bool previewing = false;
//     if (m_currentlyPreviewingIndex != Cue::kNoHotCue) {
//         if (!newPlay && oldPlay) {
//             // play latch request: stop previewing and go into normal play
//             mode. int oldPreviewingIndex =
//                     m_currentlyPreviewingIndex.fetchAndStoreRelease(
//                             Cue::kNoHotCue);
//             if (oldPreviewingIndex >= 0 && oldPreviewingIndex <
//             m_iNumHotCues) {
//                 HotcueControl* pLastControl =
//                 m_hotcueControls.at(oldPreviewingIndex); mixxx::CueType
//                 lastType = pLastControl->getPreviewingType(); if (lastType !=
//                 mixxx::CueType::Loop) {
//                     CuePointer pLastCue(pLastControl->getCue());
//                     if (pLastCue && pLastCue->getType() !=
//                     mixxx::CueType::Invalid) {
//                         pLastControl->setStatus(HotcueControl::Status::Set);
//                     }
//                 }
//             }
...
//                 m_pPlayIndicator->setBlinkValue(ControlIndicator::OFF);
//             } else {
//                 // Flashing indicates that a following play would move cue
//                 point m_pPlayIndicator->setBlinkValue(
//                         ControlIndicator::RATIO1TO1_500MS);
//             }
...
//                     move Cue point m_pCueIndicator->setBlinkValue(
//                             ControlIndicator::RATIO1TO1_500MS);
//                 } else if (cueMode == CueMode::MixxxNoBlinking) {
//                     m_pCueIndicator->setBlinkValue(ControlIndicator::OFF);
//                 } else {
//                     // in Pioneer mode Cue Button is flashing fast if CUE
//                     will move Cue point m_pCueIndicator->setBlinkValue(
//                             ControlIndicator::RATIO1TO1_250MS);
//                 }
//             } else {
//                 m_pCueIndicator->setBlinkValue(ControlIndicator::OFF);
...
// CueControl::TrackAt CueControl::getTrackAt() const {
//     FrameInfo info = frameInfo();
//     // Note: current can be in the padded silence after the track end >
//     total. if (info.trackEndPosition.isValid() && info.currentPosition >=
//     info.trackEndPosition) {
//         return TrackAt::End;
//     }
//     const auto mainCuePosition =
//             mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(
//                     m_pCuePoint->get());
//     if (mainCuePosition.isValid() && fabs(info.currentPosition -
//     mainCuePosition) < 0.5) {
//         return TrackAt::Cue;
//     }
//     return TrackAt::ElseWhere;
...
//             mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(
//                     m_pClosestBeat->get());
//     // Note: closestBeat can be an interpolated beat past the end of the
//     track,
//     // which cannot be reached.
//     if (closestBeat.isValid() && info.trackEndPosition.isValid() &&
//             closestBeat <= info.trackEndPosition) {
...
// }
//
// mixxx::audio::FramePos CueControl::quantizeCuePoint(mixxx::audio::FramePos
// position) {
//     // Don't quantize unset cues.
//     if (!position.isValid()) {
//         return mixxx::audio::kInvalidFramePos;
...
//
//     if (position > trackEndPosition) {
//         // This can happen if the track length has changed or the cue was set
//         in the
//         // the padded silence after the track.
//         position = trackEndPosition;
//     }
...
//
//     const auto quantizedPosition = pBeats->findClosestBeat(position);
//     // The closest beat can be an unreachable interpolated beat past the end
//     of
//     // the track.
//     if (quantizedPosition.isValid() && quantizedPosition <= trackEndPosition)
//     {
...
//
// SeekOnLoadMode CueControl::getSeekOnLoadPreference() {
//     return getConfig()->getValue(ConfigKey("[Controls]", "CueRecall"),
...
// }
//
// void CueControl::setCurrentSavedLoopControlAndActivate(HotcueControl*
// pControl) {
//     HotcueControl* pOldSavedLoopControl =
//     m_pCurrentSavedLoopControl.fetchAndStoreAcquire(nullptr); if
//     (pOldSavedLoopControl && pOldSavedLoopControl != pControl) {
//         // Disable previous saved loop
//         DEBUG_ASSERT(pOldSavedLoopControl->getStatus() !=
//         HotcueControl::Status::Empty);
//         pOldSavedLoopControl->setStatus(HotcueControl::Status::Set);
//     }
//
...
//     }
//
//     DEBUG_ASSERT(pSavedLoopControl->getStatus() !=
//     HotcueControl::Status::Empty); DEBUG_ASSERT(pSavedLoopControl->getCue()
//     &&
//             pSavedLoopControl->getCue()->getPosition() ==
//                     mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(
//                             m_pLoopStartPosition->get()));
...
//
//     DEBUG_ASSERT(pSavedLoopControl->getStatus() ==
//     HotcueControl::Status::Active); pCue->setStartPosition(startPosition);
//     pCue->setEndPosition(endPosition);
//     DEBUG_ASSERT(pSavedLoopControl->getStatus() ==
//     HotcueControl::Status::Active);
// }
...
// int CueControl::getHotcueFocusIndex() const {
//     return
//     hotcueNumberToHotcueIndex(static_cast<int>(m_pHotcueFocus->get()));
// }
//
// ConfigKey HotcueControl::keyForControl(const QString& name) {
//     ConfigKey key;
...
//           m_pCue(nullptr) {
//     m_hotcuePosition =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("position")));
//     connect(m_hotcuePosition.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcuePositionChanged,
...
//
//     m_pHotcueStatus =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("status")));
//     m_pHotcueStatus->setReadOnly();
//
//     // Add an alias for the legacy hotcue_X_enabled CO
//     m_pHotcueStatus->addAlias(keyForControl(QStringLiteral("enabled")));
//
//     m_hotcueStem1vol =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("stem1vol")));
//     m_hotcueStem2vol =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("stem2vol")));
//     m_hotcueStem3vol =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("stem3vol")));
//     m_hotcueStem4vol =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("stem4vol")));
//     m_hotcueStem1vol->setReadOnly();
//     m_hotcueStem2vol->setReadOnly();
//     m_hotcueStem3vol->setReadOnly();
//     m_hotcueStem4vol->setReadOnly();
//
//     m_hotcueType =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("type")));
//     m_hotcueType->setReadOnly();
//
//     // The rgba value  of the color assigned to this color.
//     m_hotcueColor =
//     std::make_unique<ControlObject>(keyForControl(QStringLiteral("color")));
//     m_hotcueColor->connectValueChangeRequest(
//             this,
//             &HotcueControl::slotHotcueColorChangeRequest,
//             Qt::DirectConnection);
//
//     m_hotcueSet =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("set")));
//     connect(m_hotcueSet.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueSet,
...
//
//     m_hotcueSetCue =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("setcue")));
//     connect(m_hotcueSetCue.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueSetCue,
...
//
//     m_hotcueSetLoop =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("setloop")));
//     connect(m_hotcueSetLoop.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueSetLoop,
...
//
//     m_hotcueGoto =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("goto")));
//     connect(m_hotcueGoto.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueGoto,
...
//             Qt::DirectConnection);
//
//     // Enable/disable the loop associated with this hotcue (either a saved
//     loop
//     // or a beatloop from the hotcue position if this is a regular hotcue).
//     m_hotcueCueLoop =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("cueloop")));
//     connect(m_hotcueCueLoop.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueCueLoop,
...
//
//     m_hotcueClear =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("clear")));
//     connect(m_hotcueClear.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueClear,
...
//
//     m_hotcueSwap =
//     std::make_unique<ControlPushButton>(keyForControl(QStringLiteral("swap")));
//     connect(m_hotcueSwap.get(),
//             &ControlObject::valueChanged,
//             this,
//             &HotcueControl::slotHotcueSwap,
...
// void HotcueControl::slotHotcueColorChangeRequest(double newColor) {
//     if (newColor < 0 || newColor > 0xFFFFFF) {
//         qWarning() << "slotHotcueColorChangeRequest got invalid value:" <<
//         newColor; return;
//     }
//     // qDebug() << "HotcueControl::slotHotcueColorChangeRequest" << newColor;
//     if (!m_pCue) {
...
// mixxx::audio::FramePos HotcueControl::getPosition() const {
//     return
//     mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(m_hotcuePosition->get());
// }
//
// mixxx::audio::FramePos HotcueControl::getEndPosition() const {
//     return
//     mixxx::audio::FramePos::fromEngineSamplePosMaybeInvalid(m_hotcueEndPosition->get());
// }
//
// void HotcueControl::setCue(const CuePointer& pCue) {
//     DEBUG_ASSERT(!m_pCue);
...
//
// void HotcueControl::resetCue() {
//     // clear pCue first because we have a null check for valid data else
//     where
//     // in the code
//     m_pCue.reset();
//     setPosition(mixxx::audio::kInvalidFramePos);
...
//
// HotcueControl::Status HotcueControl::getStatus() const {
//     // Cast to int before casting to the int-based enum class because MSVC
//     will
//     // throw a hissy fit otherwise.
//     return static_cast<Status>(static_cast<int>(m_pHotcueStatus->get()));
// }
