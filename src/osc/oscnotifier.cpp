...
void OscNotifier::observeControls() {
    ...
    for (int channel = 1; channel <= 4; ++channel) {
        ...
        // Continuous
        specs.push_back({G("[Channel%1]"), "volume", Category::Continuous, std::nullopt, std::nullopt});
        specs.push_back({G("[Channel%1]"), "pregain", Category::Continuous, std::nullopt, std::nullopt});
        specs.push_back({G("[Channel%1]"), "rate", Category::Continuous, std::nullopt, std::nullopt});
        specs.push_back({G("[Channel%1]"), "bpm", Category::Continuous, Policy::Throttle, 100}); // example override: bpm throttle 100ms

        specs.push_back({G("[QuickEffectRack1_[Channel%1]]"), "enabled", Category::Toggle, std::nullopt, std::nullopt});
        specs.push_back({G("[QuickEffectRack1_[Channel%1]]"), "super1", Category::Continuous, std::nullopt, std::nullopt});

        // EQs
        for (const auto* k : {"parameter1", "parameter2", "parameter3"})
            specs.push_back({G("[EqualizerRack1_[Channel%1]_Effect1]"), k, Category::Continuous, std::nullopt, std::nullopt});
        for (const auto* k : {"button_parameter1", "button_parameter2", "button_parameter3"})
            specs.push_back({G("[EqualizerRack1_[Channel%1]_Effect1]"), k, Category::Toggle, std::nullopt, std::nullopt});

        // Stems
        for (int stem = 1; stem <= 4; ++stem) {
            auto GS = [channel, stem](const char* fmt) { return QString(fmt).arg(channel).arg(stem); };
            specs.push_back({GS("[Channel%1_Stem%2]"), "mute", Category::Toggle, std::nullopt, std::nullopt});
            specs.push_back({GS("[Channel%1_Stem%2]"), "volume", Category::Continuous, std::nullopt, std::nullopt});
            specs.push_back({GS("[QuickEffectRack1_[Channel%1_Stem%2]]"), "enabled", Category::Toggle, std::nullopt, std::nullopt});
            specs.push_back({GS("[QuickEffectRack1_[Channel%1_Stem%2]]"), "super1", Category::Continuous, std::nullopt, std::nullopt});
        }
    }

    // Master
    specs.push_back({QStringLiteral("[Master]"), "crossfader", Category::Continuous, std::nullopt, std::nullopt});
...
static Category inferCategory(const QString& key) {
    if (key == "play" || key == "pfl" || key == "enabled" || key == "mute" || key.startsWith("button_"))
