        ...
        if (max > 0.0f) {
            ...
            case Qt::AlignRight:
                painter->drawLine(x,
                        ...
                        breadth -
                                (int)(heightFactor *
                                        sqrtf(math_max(maxAll, maxAllNext))));
                break;
            case Qt::AlignTop:
            case Qt::AlignLeft:
                painter->drawLine(
                        x, 0, x, (int)(heightFactor * sqrtf(math_max(maxAll, maxAllNext))));
                break;
            default:
                painter->drawLine(x,
                        (int)(halfBreadth - heightFactor * sqrtf(maxAll)),
                        x,
                        (int)(halfBreadth + heightFactor * sqrtf(maxAllNext)));
            }
        }
    }
}
