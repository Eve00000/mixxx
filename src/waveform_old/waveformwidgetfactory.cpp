...
WaveformWidgetFactory::WaveformWidgetFactory()
    ...
    if (pGlWidget && pGlWidget->isValid()) {
        ...
        if (QOpenGLContext::currentContext()) {
            ...
            qDebug() << QString("openGLVersionFlags 0x%1").arg(version, 0, 16)
...
bool WaveformWidgetFactory::setConfig(UserSettingsPointer config) {
    ...
    int beatGridAlpha = m_config->getValue(
            ConfigKey("[Waveform]", "beatGridAlpha"), m_beatGridAlpha);
    ...
    for (int i = 0; i < FilterCount; i++) {
        double visualGain = m_config
                                    ->getValueString(ConfigKey("[Waveform]",
