...
//         qWarning() << "Can't add skin-created control" << key << "to set,
//         control will be leaked!"; return controlButton;
//     }
...
// }
//
// ControlObject* LegacySkinParser::controlFromConfigNode(const QDomElement&
// element,
...
//
//     bool bPersist = m_pContext->selectAttributeBool(keyElement, "persist",
//     false);
//
...
//                  << "line:" << parseResult.errorLine << "column:" <<
//                  parseResult.errorColumn;
//         qDebug() << "LegacySkinParser::openSkin - message:" <<
...
//         while (!scheme.isNull()) {
//             if (scheme.isElement()) {
//                 QString schemeName = XmlParse::selectNodeQString(scheme,
//                 "Name"); schemeList.append(schemeName);
//             }
...
// }
//
// SkinManifest LegacySkinParser::getSkinManifest(const QDomElement&
// skinDocument) {
...
//         attributes_node.toElement().elementsByTagName("attribute"); for (int
//         i = 0; i < attribute_nodes.count(); ++i) {
...
//                 QString configKey =
//                 attribute_element.attribute("config_key"); QString persist =
//                 attribute_element.attribute("persist"); QString value =
//                 attribute_element.text(); SkinManifest::Attribute* attr =
//                 manifest.add_attribute();
...
// }
//
// QWidget* LegacySkinParser::parseSkin(const QString& skinPath, QWidget*
// pParent) {
//     ScopedTimer timer(QStringLiteral("SkinLoader::parseSkin"));
//     qDebug() << "LegacySkinParser loading skin:" << skinPath;
//
//     // Remove all widget pointers we previously registered for shortcut
//     tooltips. m_pKeyboard->clearWidgets();
//
//     m_pContext = std::make_unique<SkinContext>(m_pConfig, skinPath +
//     "/skin.xml"); m_pContext->setSkinBasePath(skinPath);
//
//     if (m_pParent) {
//         qDebug()
//                 << "ERROR: Somehow a parent already exists -- you are
//                 probably "
...
//         bool ok = false;
//         double value =
//         QString::fromStdString(attribute.value()).toDouble(&ok); if (!ok) {
//             SKIN_WARNING(skinDocument,
//                     *m_pContext,
//                     QStringLiteral("Failed reading double value from skin
//                     attribute: %1")
...
//     // Note: Here we assume this path exists. If it doesn't
//     SkinLoader::getSkinSearchPaths()
//     // would have already triggered an error message.
//     // Note: we may also add the user skins path, in case there are custom
//     skins
//     // that use the same template inheritance scheme like official skins, but
//     we
//     // don't because unfortunately there is no reliable way to apply
//     equivalent
...
//
//     if (widgets.empty()) {
//         SKIN_WARNING(skinDocument, *m_pContext, QStringLiteral("Skin produced
//         no widgets!")); return nullptr;
...
// }
//
// LaunchImage* LegacySkinParser::parseLaunchImage(const QString& skinPath,
// QWidget* pParent) {
//     m_pContext = std::make_unique<SkinContext>(m_pConfig, skinPath +
//     "/skin.xml"); m_pContext->setSkinBasePath(skinPath);
//
...
//     LaunchImage* pLaunchImage = new LaunchImage(pParent,
//     stylesheetAbsIconPaths(styleSheet)); setupSize(skinDocument,
//     pLaunchImage); return pLaunchImage;
// }
//
...
//     if (nodeName == "skin") {
//         // Parent all the skin widgets to an inner QWidget (this was
//         MixxxView
//         // in <=1.8, MixxxView was a subclass of QWidget), and then wrap it
//         in
//         // an outer widget. The Background parser parents the background
//         image
//         // to the inner widget but then sets the fill color of the outer
//         widget
//         // so that fullscreen will expand with the right color to fill in the
//         // non-background areas. We put the inner widget in a layout inside
//         the
...
//             QDomElement background = m_pContext->selectElement(node,
//             "Background"); if (!background.isNull()) {
//                 parseBackground(background, pOuterWidget, pInnerWidget);
//             }
//
//             // Interpret <Size>, <SizePolicy>, <Style>, etc. tags for the
//             root node. setupWidget(node, pInnerWidget, false);
//
...
//             SKIN_WARNING(node,
//                     *m_pContext,
//                     QStringLiteral("Scrollables must have exactly one
//                     child"));
//         }
//
...
//     // Note: if we call setupConnections earlier and it sets the visible
//     property
...
//     ControlObject* pNextControl = controlFromConfigNode(node.toElement(),
//     "NextControl"); ConfigKey nextConfigKey; if (pNextControl != nullptr) {
//         nextConfigKey = pNextControl->getKey();
//     }
//
//     ControlObject* pPrevControl = controlFromConfigNode(node.toElement(),
//     "PrevControl"); ConfigKey prevConfigKey; if (pPrevControl != nullptr) {
...
//         bool persist = m_pContext->selectAttributeBool(node, "persist",
//         false); pCurrentPageControl = controlFromConfigKey(configKey,
//         persist); if (pCurrentPageControl != nullptr) {
...
//                 ConfigKey configKey =
//                 ConfigKey::parseCommaSeparated(trigger_configkey); pControl =
//                 controlFromConfigKey(configKey, false);
//             }
...
//     if (background != nullptr && !background->isNull()) {
//         pInnerWidget->setFixedSize(background->width(),
//         background->height());
//         pOuterWidget->setMinimumSize(background->width(),
//         background->height());
//     }
//
...
// }
//
// void LegacySkinParser::setupLabelWidget(const QDomElement& element, WLabel*
// pLabel) {
//     // NOTE(rryan): To support color schemes, the WWidget::setup() call must
//     // come first. This is because WLabel derivatives change the palette
//     based
//     // on the node and setupWidget() will set the widget style. If the style
//     is
...
////     BaseTrackPlayer* pPlayer = m_pPlayerManager->getPlayer(group);
////     if (!pPlayer) {
////         SKIN_WARNING(node, *m_pContext, QStringLiteral("No player found for
/// group: %1").arg(group)); /         return nullptr; /     } /     WBpmEditor*
/// pBpmEditor = new WBpmEditor(group, m_pParent); /     pBpmEditor->setup(node,
///*m_pContext); /     commonWidgetSetup(node, pBpmEditor); / Set tooltips of
/// child widgets for mode selection & editing /     const QString tapTooltip =
/// m_tooltips.tooltipForId("tempo_tap_bpm_tap"); /     const QString
/// editTooltip
///= m_tooltips.tooltipForId("tempo_edit"); /
/// pBpmEditor->setTapButtonTooltip(tapTooltip); /
/// pBpmEditor->setEditButtonTooltip(editTooltip); /     return pBpmEditor; / }
//
...
//    if (!pPlayer) {
//        SKIN_WARNING(node, *m_pContext, QStringLiteral("No player found for
//        group: %1").arg(group)); return nullptr;
//    }
//
...
//    connect(overviewWidget, &WOverview::cloneDeck, m_pPlayerManager,
...
//    connect(pPlayer, &BaseTrackPlayer::newTrackLoaded, overviewWidget,
//    &WOverview::slotTrackLoaded); connect(pPlayer,
//    &BaseTrackPlayer::loadingTrack, overviewWidget,
...
//    }
//
//    WWaveformViewer* viewer = new WWaveformViewer(group, m_pConfig,
//    m_pParent); viewer->setSizePolicy(QSizePolicy::Expanding,
//    QSizePolicy::Expanding); WaveformWidgetFactory* pFactory =
//    WaveformWidgetFactory::instance(); pFactory->setWaveformWidget(viewer,
//    node, *m_pContext);
//
...
//            m_pPlayerManager,
//            &PlayerManager::slotLoadLocationToPlayerMaybePlay);
//    connect(viewer, &WWaveformViewer::cloneDeck, m_pPlayerManager,
//    &PlayerManager::slotCloneDeck);
//
...
//    ControlObject* valueControl = controlFromConfigNode(node.toElement(),
//    "Value");
//
//    ConfigKey configKey;
...
//    WRecordingDuration* p = new WRecordingDuration(m_pParent,
//    m_pRecordingManager); setupBaseWidget(node, p); setupWidget(node, p);
//    p->setup(node, *m_pContext);
//    setupConnections(node, p);
//    p->installEventFilter(m_pKeyboard);
//    p->installEventFilter(m_pControllerManager->getControllerLearningEventFilter());
...
//        group: %1").arg(group)); return nullptr;
//    }
//    // Note: For some reasons on X11 we need to create the widget without a
//    parent to avoid to
//    // create two platform windows (QXcbWindow) in QWidget::create() for this
//    widget.
//    // This happens, because the QWidget::create() of a parent() will populate
//    all children
//    // with platform windows q_createNativeChildrenAndSetParent() while
//    another window is already
//    // under construction. The ID for the first window is not cleared and
//    leads to a segfault
//    // during on shutdown. This has been tested with Qt 5.12.8 and 5.15.3
//    QWidget* pParent = (qApp->platformName() == QLatin1String("xcb")) ?
//    nullptr : m_pParent; WSpinnyBase* pSpinny;
// #ifdef MIXXX_USE_QOPENGL
//    if (pWaveformWidgetFactory->isOpenGlShaderAvailable() &&
//            !CmdlineArgs::Instance().getUseLegacySpinny()) {
//        pSpinny = new WSpinnyGLSL(pParent, group, m_pConfig, m_pVCManager,
//        pPlayer);
//    } else
// #endif
//    {
//        pSpinny = new WSpinny(pParent, group, m_pConfig, m_pVCManager,
//        pPlayer);
//    }
//    if (!pParent) {
...
//            m_pPlayerManager,
//            &PlayerManager::slotLoadLocationToPlayerMaybePlay);
//    connect(pSpinny, &WSpinnyBase::cloneDeck, m_pPlayerManager,
//    &PlayerManager::slotCloneDeck);
//
//    ControlObject* showCoverControl = controlFromConfigNode(node.toElement(),
//    "ShowCoverControl"); ConfigKey configKey; if (showCoverControl) {
...
//                    !pWaveformWidgetFactory->isOpenGlesAvailable())) {
//        // Legacy WVuMeter
//        WVuMeterLegacy* pVuMeterWidget =
//        parseStandardWidget<WVuMeterLegacy>(node);
...
//    // during on shutdown. This has been tested with Qt 5.12.8 and 5.15.3
//    QWidget* pParent = (qApp->platformName() == QLatin1String("xcb")) ?
//    nullptr : m_pParent; WVuMeterBase* pVuMeterWidget;
// #ifdef MIXXX_USE_QOPENGL
//    if (pWaveformWidgetFactory->isOpenGlShaderAvailable()) {
...
//    m_pConfig); commonWidgetSetup(node, pLineEditSearch, false);
...
//            SKIN_WARNING(node,
//                    *m_pContext,
//                    QStringLiteral("No player found for group:
//                    %1").arg(group));
//            return nullptr;
//        }
//    }
//    WCoverArt* pCoverArt = new WCoverArt(m_pParent, m_pConfig, group,
//    pPlayer); commonWidgetSetup(node, pCoverArt); pCoverArt->setup(node,
//    *m_pContext);
//
//    // If no group was provided, hook the widget up to the Library.
//    if (group.isEmpty()) {
//        // Connect cover art signals to the library
//        connect(m_pLibrary, &Library::switchToView, pCoverArt,
//        &WCoverArt::slotReset); connect(m_pLibrary,
//        &Library::enableCoverArtDisplay, pCoverArt, &WCoverArt::slotEnable);
//        connect(m_pLibrary, &Library::trackSelected, pCoverArt,
//        &WCoverArt::slotLoadTrack);
//    } else if (pPlayer != nullptr) {
//        connect(pCoverArt,
//                &WCoverArt::trackDropped,
//                m_pPlayerManager,
//                &PlayerManager::slotLoadLocationToPlayerMaybePlay);
//        connect(pCoverArt, &WCoverArt::cloneDeck, m_pPlayerManager,
//        &PlayerManager::slotCloneDeck);
//    }
//
//    return pCoverArt;
...
//    QWidget* pChild = child_widgets[0];
//    if (pChild == nullptr) {
//        SKIN_WARNING(node, *m_pContext, QStringLiteral("SingletonDefinition
//        child widget is NULL")); return;
//    }
//
...
//
//    // This must come after the bindLibraryWidget or we will not style any of
//    the
...
// QWidget* LegacySkinParser::parseLibraryPreparationWindow(const QDomElement&
// node) {
...
//     BaseTrackTableModel::setApplyPlayedTrackColor(applyPlayedTrackColor);
//
//     m_pLibrary->bindLibraryPreparationWindowWidget(pLibraryPreparationWindowWidget,
//     m_pKeyboard);
//
//     // This must come after the bindLibraryWidget or we will not style any of
//     the
...
//     QWidget* pLibraryPreparationWindowWidget =
//     parseLibraryPreparationWindow(node);
//
...
//         ").arg(color.name())); styleHack.append(QString("WSearchLineEdit {
//         color: %1; }\n ").arg(color.name()));
//         styleHack.append(QString("QTextBrowser { color: %1; }\n
//         ").arg(color.name())); styleHack.append(QString("QLabel { color: %1;
//         }\n ").arg(color.name())); styleHack.append(QString("QRadioButton {
//         color: %1; }\n ").arg(color.name()));
...
//         }\n ").arg(color.name())); styleHack.append(QString("QTextBrowser {
//         background-color: %1; }\n ").arg(color.name()));
...
//     if (m_pContext->hasNodeSelectString(node, "BgColorRowEven",
//     &bgColorRowEven)) {
...
//     if (m_pContext->hasNodeSelectString(node, "BgColorRowUneven",
//     &bgColorRowUneven)) {
...
//     if (!parseResult) {
//         qWarning() << "LegacySkinParser::loadTemplate - setContent failed
//         see"
...
//         qWarning() << "LegacySkinParser::loadTemplate - setContent failed
//         see"
//                    << absolutePath << "line:" << errorLine << "column:" <<
//                    errorColumn;
//         qWarning() << "LegacySkinParser::loadTemplate - message:" <<
//         errorMessage;
// #endif
...
//         SKIN_WARNING(node,
//                 *m_pContext,
//                 QStringLiteral("Template instantiation for template failed:
//                 %1")
//                         .arg(path));
...
//
//     // If the group is not present, then check for a Channel, since legacy
//     skins
...
//     // For HotcueButton, widget connections are created in its own setup(),
//     not
...
//     // ControllerLearningEventFilter doesn't (yet) care about keyboard events
//     or
//     // modifiers, so let's use just right-click to make this learnable as
//     well. pCLEFilter->addWidgetClickInfo(pWidget->toQWidget(),
//             Qt::RightButton,
//             controlFromConfigKey(pWidget->getClearConfigKey(), false),
//             ControlParameterWidgetConnection::EmitOption::EMIT_ON_PRESS_AND_RELEASE);
//
//     // The list of ConfigKeys and translatable command strings for the
//     tooltip
...
//     m_pEffectsManager); setupLabelWidget(node, pEffectChain); return
//     pEffectChain;
// }
//
// QWidget* LegacySkinParser::parseEffectChainPresetButton(const QDomElement&
// node) {
//     WEffectChainPresetButton* pButton = new
//     WEffectChainPresetButton(m_pParent, m_pEffectsManager);
//     commonWidgetSetup(node, pButton);
//     QString toolTip =
//     m_tooltips.tooltipForId("EffectUnit_chain_preset_menu"); if
//     (!toolTip.isEmpty()) {
...
// }
//
// QWidget* LegacySkinParser::parseEffectChainPresetSelector(const QDomElement&
// node) {
...
//     WEffectSelector* pSelector = new WEffectSelector(m_pParent,
//     m_pEffectsManager); commonWidgetSetup(node, pSelector);
//     pSelector->setup(node, *m_pContext);
...
// }
//
// QWidget* LegacySkinParser::parseEffectParameterKnobComposed(const
// QDomElement& node) {
...
//     m_pEffectsManager); commonWidgetSetup(element, pWidget);
//     pWidget->setup(element, *m_pContext);
//     pWidget->installEventFilter(m_pKeyboard);
//     pWidget->installEventFilter(
//             m_pControllerManager->getControllerLearningEventFilter());
...
//             SKIN_WARNING(node,
//                     *m_pContext,
//                     QStringLiteral("Could not parse horizontal size policy:
//                     %1")
//                             .arg(xs));
//         }
//
...
//             SKIN_WARNING(node,
//                     *m_pContext,
//                     QStringLiteral("Could not parse vertical size policy:
//                     %1")
//                             .arg(ys));
//         }
//
...
//             QString platformSpecificSrc =
//             styleElement.attribute(platformSpecificAttribute); QFile
//             platformSpecificFile(platformSpecificSrc); if
//             (platformSpecificFile.open(QIODevice::ReadOnly)) {
...
//         // This should help to scale the GUI along with the native widgets
//         once
//         // we are on Qt 5.
...
//     }
//
//     // Legacy fixes: In Mixxx <1.12.0 we used QGroupBox for WWidgetGroup.
//     Some
...
//     // setupWidget since a BindProperty connection to the display parameter
//     can
//     // cause the widget to be polished (i.e. style computed) before it is
//     // ready. The most common case is that the object name has not yet been
//     set
...
//     } else if (m_pContext->hasNodeSelectString(node, "TooltipId",
//     &toolTipId)) {
//         toolTip = m_tooltips.tooltipForId(toolTipId);
//         if (!toolTip.isEmpty()) {
//             pBaseWidget->prependBaseTooltip(toolTip);
//         } else if (!toolTipId.isEmpty()) {
//             // Only warn if there was a tooltip ID specified and no tooltip
//             for
//             // that ID.
//             SKIN_WARNING(node,
//                     *m_pContext,
...
// }
//
// void LegacySkinParser::setupConnections(const QDomNode& node, WBaseWidget*
// pWidget) {
...
//         ControlObject* control = controlFromConfigNode(con.toElement(),
//         "ConfigKey");
//
//         if (control == nullptr) {
//             continue;
...
//             int directionOption =
//             ControlParameterWidgetConnection::DIR_FROM_AND_TO_WIDGET; if
//             (m_pContext->hasNodeSelectBool(
...
//             case Qt::RightButton:
//                 pWidget->addConnection(std::move(pConnection),
//                 WBaseWidget::ConnectionSide::Right); break;
//             default:
//                 // can't happen. Nothing else is returned by
//                 parseButtonState(); DEBUG_ASSERT(false); break;
//             }
//
...
//             ControlParameterWidgetConnection::DIR_FROM_WIDGET) {
//                 m_pControllerManager->getControllerLearningEventFilter()
//                         ->addWidgetClickInfo(pWidget->toQWidget(), state,
//                         control,
...
//                     shortcutKeys.append(std::make_pair(control->getKey(),
//                     QString()));
//                 } else if ((pSlider = qobject_cast<const
//                 WSliderComposed*>(pWidget->toQWidget())) ||
//                         qobject_cast<const
//                         WKnobComposed*>(pWidget->toQWidget())) {
//                     const ConfigKey cfgKey = control->getKey();
//                     // Add up/down controls with orientation-dependent tr
//                     strings if (pSlider && pSlider->tryParseHorizontal(node))
//                     {
...
//                         shortcutKeys.emplace_back(
//                                 subKey(cfgKey,
//                                 QStringLiteral("_down_small")), tr("left
//                                 small"));
//                         shortcutKeys.emplace_back(
...
//                         shortcutKeys.emplace_back(
//                                 subKey(cfgKey,
//                                 QStringLiteral("_down_small")), tr("down
//                                 small"));
//                         shortcutKeys.emplace_back(
//                                 subKey(cfgKey, QStringLiteral("_up")),
//                                 tr("up"));
...
//         // Check if the selected scheme has a <LaunchImageStyle> node with a
//         string schemeLaunchImageStyle =
//         m_pContext->selectString(colorSchemeNode, "LaunchImageStyle");
//     }
...
//     // Initially, this was a Workaround for
//     https://bugs.kde.org/show_bug.cgi?id=434451
//     // which renders relative SVG icon paths in external stylesheets
//     unusable:
//     // Replaces relative icon urls in stylesheets (external qss or inline
//     // <Style> nodes) with absolute file paths.
//     // Now this also replaces the 'skins:' alias which allows skin mods in
//     the
//     // user skins directory to use image urls referencing files in the system
//     // skins directory for the launch image style.
//     style.replace("url(skins:", "url(" + m_pConfig->getResourcePath() +
//     "skins/"); return style.replace("url(skin:", "url(" +
//     m_pContext->getSkinBasePath());
// }
//
    ...
    for (QDomNode con = m_pContext->selectNode(node, "Connection");
        ...
        if (m_pContext->hasNodeSelectString(con, "BindProperty", &property)) {
            ...
            if (directionOption & ControlParameterWidgetConnection::DIR_FROM_WIDGET) {
                m_pControllerManager->getControllerLearningEventFilter()
                        ->addWidgetClickInfo(pWidget->toQWidget(),
                                state,
                                control,
                                static_cast<ControlParameterWidgetConnection::
                                                EmitOption>(emitOption));
