function unzipTo {
    ...
    if ($PSVersionTable.PSVersion.Major -ge 3) {
        ...
        Add - Type - AssemblyName System.IO.Compression Add - Type -
                AssemblyName System.IO.Compression.FileSystem

                        $archiveFile = [System.IO.File] ::Open(
                $archiveFilePath, [System.IO.FileMode] ::Open) $archive =
                [System.IO.Compression.ZipArchive] ::new ($archiveFile)
#If the directory exists,               \
        unzip item by itep to force the \
                extraction.Else it would fail if file already exists.
            ...
            foreach ($item in $archive.Entries) {
                ...
                if ($destinationItemPath -like '*/') {
                    ...
                    New - Item $destinationItemPath - Force - ItemType File >
                            $null
...
#We are on powerwhell 2. We try using the system                          \
        .Note that this requires Windows 7 since it uses the Explorer zip \
                capability.
...
#This is as slow as the Shell.Application.CopyHere, \
        so the System.IO.ExtractToDirectory is favoured
